...
Lane HF19-A, MODE: DISCOVER (with the obstruction proved for every class
considered), 2026-09-05.  Base commit `5aefa82` (navier), manuscript
`navier-paper` at `39ccb66`, section `sec:quotient`.  Audited inputs used
without re-proof: `hf17-quotient-functional.md`, `hf17-quotient-evolution.md`
(both PASS), `hf18-hodge-regularity.md` (SHA-256 `e5fdbc39…2db61`, PASS with
S1–S3 applied), `hf18-divergence-speed-link.md` (SHA-256 `409421f2…62a66`,
PASS r2 with S1–S4 applied).  Source tags: [DI] directly inspected in this
programme, [MO] metadata only, never load-bearing.  No primary web source was
needed: every imported statement is a manuscript lemma or an audited note.

**MODE / RESULT: DISCOVER, negative with two exact by-products.**  A temporal
normal form \(K=\tfrac d{dt}B(u)+R\) for the transport term of the cubic
gradient quotient is shown to be *exactly equivalent* to the existence of a
coercive critical Lyapunov-type functional \(\Phi=\mathcal Q-B\), for every
functional \(B\) whatsoever (Proposition 0.2); so the only content of any
normal form lies in the explicit \(B\) for which \(\Phi'\) can be computed.
Three explicit classes are computed in full.  (i) \(B=\mathcal Q-X/3\)
(the distance to the plain cubic norm) gives the exact identity
\(K-P_3=\tfrac d{dt}(\mathcal Q-X/3)+\nu\,(D_3(w)-D_3(u))\), proving that the
HIGH-STRAIN and HIGH-PRESSURE gaps are equivalent at \(\theta\le1\) and that
they differ instantaneously by a total derivative of a \(\mathcal Q\)-controlled
functional plus a dissipation difference of unproved sign (Theorem 1).  (ii)
The heat inverse \(B_h(u)=-\int_0^\infty K(G_su)\,ds\) is rigorously defined
with the exact bound \(|B_h|\le\tfrac{3C_*}{4\nu}\mathcal Q^{4/3}\) and the
rigorous heat-direction identity \(\tfrac d{dt}B_h(G_tu)=K(G_tu)\); every
\(C^1\) scalar \(F(\mathcal Q,B_h)\) with coefficientwise cancellation of \(K\)
is \(f(\mathcal Q-B_h)\), and \(\mathcal Q-B_h\) is not coercive on fixed
energy shells unless \(K\equiv0\) on solenoidal Schwartz fields (Theorem 2);
the saturation and truncation repairs fail exactly as in HF01/HF04, and the
...
energy \(\Phi=\int_{(0,\infty)}\mathcal Q(G_su)\,m(ds)\) is non-coercive
relative to any locally bounded function of the energy (Theorem 3).  No
functional built from \(q,A,V,\Pi_{u,A},\sigma\) that is not a value function
of the coset admits a justified time derivative (Section 4), and value
functions fall under Proposition 0.2.  The first gap is unchanged; its exact
position in each candidate is displayed with scaling.
...
continuous on compact classical intervals and
\[
 K=\tfrac d{dt}B(u)+R,\qquad
 R\le\theta\nu D_3(w)+M\mathcal Q+r(t),\quad r\ge0\ \text{input-integrable},  \tag{NF}
\]
together with a boundary condition on \(B\).

**Lemma 0.1 (what the boundary condition must be).**  Let (NF) hold on
every compact classical interval.  (a) If
\[
 B(u)\le(1-c)\,\mathcal Q(u)+a_{\rm input}\qquad\text{for some }c\in(0,1],\ a_{\rm input}<\infty, \tag{C}
\]
then (G) holds with \(A_{\rm input}=a_{\rm input}+|B(u_0)|+\int_0^Hr\) and the same
\(\theta\), \(M\).  (b) The condition \(|B|\le C\mathcal Q\) with \(C\ge1\) is
not sufficient: \(B=\mathcal Q\), \(R=\nu D_3(w)\), \(\theta=1\) satisfies (NF)
and \(|B|\le\mathcal Q\) and yields nothing.

*Proof.* (a) Integrate (NF): \(\int_0^\tau K=B(u(\tau))-B(u_0)+\int_0^\tau R
\le(1-c)\mathcal Q(\tau)+a_{\rm input}+|B(u_0)|+\theta\nu\int D_3+M\int\mathcal Q+\int r\);
then (Q1) integrated gives \(\mathcal Q(\tau)+\nu\int_0^\tau D_3(w)=\mathcal Q(0)+\int_0^\tau K\), so
\(c\,\mathcal Q(\tau)+(1-\theta)\nu\int_0^\tau D_3\le\mathcal Q(0)+A_{\rm input}+M\int_0^\tau\mathcal Q\),
and Gronwall (`lem:qe-gronwall`) bounds \(\mathcal Q\) on \([0,\min(H,T_*))\), which is (G)
(indeed more: it is the conclusion of `prop:quotient-conditional`).  (b) is (Q1). \(\square\)

So the lane's "\(|B|\le C\mathcal Q\)" must be read as (C).  The next
proposition shows that (NF)+(C) is not a reformulation that could be easier
than the Lyapunov problem: it *is* the Lyapunov problem.

**Proposition 0.2 (exact equivalence).**  Let \(B\) be any functional with
\(B\circ u\) absolutely continuous on compact classical intervals and put
\(\Phi:=\mathcal Q-B\).  Then (NF) holds with \((\theta,M,r)\) if and only if
\[
 \tfrac d{dt}\Phi(u)+(1-\theta)\nu D_3(w)\le M\mathcal Q+r\quad\text{a.e. on }[0,T], \tag{L}
\]
and (C) holds if and only if \(\Phi\ge c\,\mathcal Q-a_{\rm input}\).  Consequently
a temporal normal form for \(K\) with boundary condition (C) exists if and only
if there is a functional \(\Phi\), coercive in the sense
\(\Phi\ge c\mathcal Q-a_{\rm input}\), satisfying the Lyapunov-type inequality (L).

*Proof.* By (Q1), \(K-B'=\mathcal Q'+\nu D_3(w)-B'=\Phi'+\nu D_3(w)\), so
\(R=\Phi'+\nu D_3(w)\), and \(R\le\theta\nu D_3+M\mathcal Q+r\) is (L).  The second
equivalence is the definition of \(\Phi\). \(\square\)
...
claim: by `rem:highstrain-scope`, global continuation gives (G) with \(\theta=0\),
i.e. \(\Phi=\mathcal Q\) itself satisfies (L) with \(\theta=0\) and \(M\) the low-strain
coefficient, so "no coercive \(\Phi\) satisfies (L)" would refute NS-R3.
Second, therefore, the only honest content of a normal form is an *explicit*
\(\Phi\) whose derivative can be computed and whose sign problem is displaced to
an explicit place.  Sections 1–4 compute every explicit class suggested by
the lane and identify where the sign problem lands.

## 1. The distance \(\Phi=X/3\): the two gaps are one gap
...
**Corollary 1.1 (equivalence of the two producers at \(\theta\le1\)).**
Fix \(\nu,u_0,H\).
(a) If (G) holds for \(K\) with \((\theta_s\le1,M,A)\), then for \(0<\tau<\min(H,T_*)\)
\[
 \int_0^\tau P_3\le\nu\int_0^\tau D_3(u)-(1-\theta_s)\nu\int_0^\tau D_3(w)+M\int_0^\tau\tfrac X3+\Bigl(A+\tfrac13X(0)\Bigr)
 \le\nu\int_0^\tau D_3(u)+M\int_0^\tau\tfrac X3+A+\tfrac13X(0),
\]
i.e. the pressure absorption `hyp:absorption` holds with \(\theta_p=1\) and a
Gronwall term, which with (P1) and Gronwall gives `hyp:critical`.
(b) Conversely, if `hyp:absorption` holds with \((\theta_p\le1,A)\), then
\[
 \int_0^\tau K\le\nu\int_0^\tau D_3(w)-(1-\theta_p)\nu\int_0^\tau D_3(u)+A+\tfrac13X(0),
\]
i.e. (G) with \(\theta_s=1\), \(M=0\).
...
*Proof.* (a) From (1.1) with \(s=0\): \(\int P_3=\int K-[\mathcal Q-X/3]_0^\tau
-\nu\int(D_3(w)-D_3(u))\), and \(-[\mathcal Q-X/3]_0^\tau\le X(0)/3\) because
\(\mathcal Q(\tau)\le X(\tau)/3\) and \(\mathcal Q(0)\ge0\); use \(\mathcal Q\le X/3\)
in the Gronwall term.  (b) Symmetric, with \(-[X/3-\mathcal Q]_0^\tau\le X(0)/3\)
because \(X/3-\mathcal Q\ge0\). (c) is read off the displayed middle terms. \(\square\)
...
does not move the gap; it proves that the two graph producers HIGH-STRAIN and
HIGH-PRESSURE are the same gap up to the exact terms displayed in (1.1).
This is a strengthening of the packet's statement that both are "equivalent
to continuation": they are equivalent to each other *instantaneously up to a
total derivative and a dissipation difference*, not only through the
...
sampled fields.  If \(D_3(w)\le D_3(u)\) held in general, the strain route
would be the *sharper* of the two: (1.1) would read
\(\int K\le\int P_3+X(0)/3\) with no dissipation loss.  A proof or a
...
(`lem:quotient-stability`).  Differentiability on \((0,\infty)\): the curve is
\(C^1\) into \(H^2\subset L^3\), so Step 2 of `lem:quotient-chainrule` applies verbatim
...
for \(s>0\), so
\(\int_0^\infty|K(G_su)|ds\le\tfrac{C_*}\nu\int_0^{\mathcal Q(u)}x^{1/3}dx=\tfrac{3C_*}{4\nu}\mathcal Q(u)^{4/3}\),
the substitution being legitimate because \(s\mapsto\mathcal Q_s\) is \(C^1\),
nonincreasing, with range \((0,\mathcal Q(u)]\).  Oddness and homogeneity are
Lemma 2.1(d); dilation invariance uses \(G_s(\lambda u(\lambda\cdot))=\lambda(G_{\lambda^2s}u)(\lambda\cdot)\)
...
falsifier, already the obstruction of HF18-A Corollary 4.
...
merely-\(L^3\) minimizer".**  The obstruction below shows that even granting
it formally, the class fails.
...
(REPAIR form, accepted there): the fibres \(\mathcal Q-B=y\), \(\mathcal Q>0\) of
...
By Prop. 2.2 and (Q4), \(\mathcal Q(a\psi_{\sigma_a})-B_h(a\psi_{\sigma_a})=a^3(\mathcal Q(\psi_{\sigma_a})-aB_h(\psi_{\sigma_a}))=0\)
...
and since \(|g'(\mathcal Q)B_h|\le\tfrac{3C_*}{4}\cdot\tfrac{\mathcal Q^{4/3}/\nu}{3\mathcal Q^{2/3}(\nu+\mathcal Q^{1/3})^2}\cdot\nu\le\tfrac{C_*}{4}\),
the coefficient is at least \(1-\delta(1+C_*/4)\ge\tfrac12\) for \(\delta\le\tfrac1{2(1+C_*/4)}\).
...
\]
an input bound (HF01 (7) transferred), and \(B_a\) satisfies (C) with \(c=1\).
The formal normal form is \(K(u)=\tfrac d{dt}B_a(u)+\bigl[K(u)-K(G_au)\bigr]+\rho_a\),
\(\rho_a=DB_a(u)[\mathbb P(u\cdot\nabla)u]\).  The heat defect
\(K(u)-K(G_au)\) is not controlled: the subtracted term satisfies
\(|K(G_au)|\le C_*(\nu a)^{-1/4}\|u_0\|_2\,D_3(w(G_au))\), so it is absorbable
into \(\theta\nu D_3(w)\) *if* the (open) heat monotonicity
\(D_3(w(G_au))\le CD_3(w(u))\) held, and is otherwise an energy-controlled
low-output quantity by (2.5)-type bounds; in either case what remains is
\(K(u)\) itself up to absorbable terms: the truncation removes only the
...
corrections of \(\mathcal Q\) are excluded from having a justified derivative
by Section 4 unless they are value functions; heat-smoothed *value* functions
...
which is annihilated by \(A(G_su)\) (`lem:quotient-minimizer`(c)), and
\(\langle A(G_su),\nu\Delta G_su\rangle=-\nu D_3(w(G_su))\) (HF18-A Thm 2 at \(G_su\)).
The bounds are (Q5) and the \(L^2\to L^3\) heat bound with `prop:energy`. \(\square\)

**Theorem 3 (heat-smoothed modified energies are not coercive).**  Let \(m\)
be a finite positive Borel measure on \((0,\infty)\) (no atom at \(0\)) and
\(\Phi_m(u):=\int_{(0,\infty)}\mathcal Q(G_su)\,m(ds)\).  Then for every \(c>0\) and
every locally bounded \(a:[0,\infty)\to[0,\infty)\) there are solenoidal
Schwartz fields \(u\) with \(\Phi_m(u)<c\,\mathcal Q(u)-a(\|u\|_2^2)\).  In
particular \(B=\mathcal Q-\Phi_m\) (which includes \(B=\mathcal Q-\mathcal Q(G_{a}u)\))
violates (C) for every \(c\), \(a_{\rm input}\), although Lemma 3.1 gives it an
exact evolution.
...
be coercive.  Letting \(m\to\delta_0\) recovers the tautology of Lemma 0.1(b).
...
the minimizer \(w(u)\).  A functional \(B(u)=\Psi(u,w(u))\) has a justified time
derivative along the classical branch only through one of two mechanisms:
...
transfers (upper bound from the competitor \(w+h\), lower bound from \(w'-h\),
remainder from stability) and \(DB(u)[h]=D\Psi(w_\Psi(u))[h]\).  Examples:
...
\(L^3\)-contractive (Section 3); \(\Psi(v)=\tfrac13\int\rho|v|^3\) with a fixed weight;
\(\Psi(v)=F(v)+\langle v,\mu\rangle\) with a fixed \(\mu\in L^{3/2}\).  For every such
\(B\), Proposition 0.2 applies to \(\Phi=\mathcal Q-B\): the normal form is the
...
\(\partial_u\Psi\) term is an explicit functional of \(u\) — again Proposition 0.2.)
...
\(\|V\|_{H^1}^2\)-type corrections — has no justified time derivative, because
\(t\mapsto w(u(t))\) is known only to be \(\tfrac12\)-Hölder into \(L^3\)
(`lem:quotient-stability`) and \(t\mapsto V(u(t))\) only bounded in \(H^1\)
(HF18-A (1.9)).  A normal form using such a \(B\) hits the falsifier
"differentiating the merely-\(L^3\) minimizer" at its first step, before any
estimate.  Proving differentiability of \(u\mapsto w(u)\) (an \(L^3\)-implicit
function theorem for the degenerate \(3\)-Laplace-type equation HF18-A (1.1))
is a distinct open problem; HF18-A §1.3 records why the standard theory does
not apply.

Consequently the classes (a)–(c) contain no candidate beyond Sections 1–3.

## 5. Bounded numerical evidence (periodic proxy; static problem only)

Scratch `hf19_probe.py` (session scratchpad, ephemeral; not part of the record): on a \(2\pi\)-periodic box, \(N^3\) grid, a random
solenoidal \(u=\operatorname{curl}\psi\) with modes \(|k|\le3\), unit mean-square
amplitude; \(q=\nabla\phi\) by L-BFGS minimisation of \(\tfrac13\int|u+\nabla\phi|^3\)
in the full \(N^3\)-dimensional \(\phi\)-space (spectral derivatives); converged
...
3. \(K<0\) in all four runs and along the whole heat orbit until \(|K|\lesssim10^{-4}\);
   hence \(B_h>0\) on these fields, so the family of Theorem 2(ii) can be
   started at \(\phi=u\) directly.  The ratio \(B_h/\mathcal Q^{4/3}\) is
   \(10^{-4}\)–\(10^{-3}\), far below \(3C_*/(4\nu)\): (2.2) is a size bound, not
   sharp on these fields.  No sign of \(K\) is claimed in general
   (HF18-B §3.3(d)).
4. \(D_3(u)-D_3(w)>0\) in all four runs (\(1.8\)–\(2.6\%\) of \(D_3\)); Remark 1.3.

## 6. Frontier record

**MODE / RESULT:** DISCOVER, negative for every class considered, with the
obstruction proved per class.  Positive by-products: Proposition 0.2 (a
temporal normal form for \(K\) with the correct boundary condition (C) is
*equivalent* to a coercive functional satisfying the Lyapunov-type
inequality (L), for arbitrary \(B\)); Theorem 1 (exact identity
\(\int(K-P_3)=[\mathcal Q-X/3]+\nu\int(D_3(w)-D_3(u))\), the two graph producers
are one gap at \(\theta\le1\)); Proposition 2.2 (\(|B_h|\le\tfrac{3C_*}{4\nu}\mathcal Q^{4/3}\),
...
(heat-smoothed modified energies are never coercive).

**CLAIM AND SCOPE:** Lemma 0.1, Prop. 0.2, Theorem 1, Cor. 1.1 on every
compact classical interval of the branch of `prop:localtheory`, using only the
audited (Q1)–(Q5), (P1)–(P2); Lemma 2.1, Prop. 2.2–2.3 for every solenoidal
\(u\in H^m\), \(m\ge4\), \(\nu>0\); Theorem 2 on the ambient scalar domain
\(\Omega\) with coefficientwise cancellation as defined in (2.4), part (ii)
under the hypothesis \(B_h\not\equiv0\) (equivalently \(K\not\equiv0\), part (iii));
Cor. 2.4 formal for the chain rule, rigorous for the bounds (2.5); Lemma 3.1
and Theorem 3 unconditional.  Section 4 is a classification of which
functionals admit a justified derivative, not a theorem about all functionals.

**EVIDENCE:** `prop:quotient-evolution`, `lem:quotient-transport`,
`lem:quotient-chainrule`, `lem:quotient-heat`, `lem:quotient-stability`,
`lem:quotient-scaling`, `lem:quotient-coercive`, `prop:pressure`,
`prop:lowpressure`, `prop:energy`, `lem:qe-gronwall` [DI, manuscript];
...
At the level of Proposition 0.2 the first unsupported implication is the
existence of a coercive \(\Phi\) with (L), which is the packet's statement that
any closing mechanism is a regularity proof.

**SURVIVING CONDITIONAL SUFFIX:** (i) If either producer holds — (G) for \(K\)
with \(\theta_s\le1\), or `hyp:absorption` with \(\theta_p\le1\) — then the other
holds with \(\theta=1\) by Cor. 1.1, and `hyp:critical` follows; a strict
\(\theta<1\) transfers as soon as \(D_3(u)\) and \(D_3(w)\) are comparable in the
needed direction (Remark 1.3).  (ii) If \(u\mapsto w(u)\) were shown
differentiable (an implicit-function theorem for HF18-A (1.1)), the formal
chain rule of (2.4) would become rigorous; Theorem 2 shows this would not
help the class \(F(\mathcal Q,B_h)\), but it would open the classes of Section 4(c).
(iii) If \(D_3(w(G_su))\le CD_3(w(u))\) (heat monotonicity of the quotient
dissipation) were proved, the smoothed transport term \(K(G_au)\) in Cor. 2.4(b)
would be absorbable for an input-selected \(a\); this does not touch the
unsmoothed \(K\).
...
\(u\mapsto w(u)\); no rigorous transport-direction chain rule for \(B_h\); no
obstruction for the general class of functionals (Prop. 0.2 shows none is
provable without deciding NS-R3); no HIGH-STRAIN, HIGH-PRESSURE, critical,
continuation, or regularity result; no novelty claim for the heat-inverse
device (HF01/HF04) or the characteristic argument (HF04 review); no
statement about which \(w\) occur along trajectories from the periodic
numerics.  NS-R3 remains OPEN.

**NEXT DISTINCT ACTION:** two well-posed static questions fall out of
Theorem 1 and Cor. 2.4, both on the linear test class of HF18-B Prop. 1.4
...
(2) FALSIFY or prove heat monotonicity \(D_3(w(G_su))\le CD_3(w(u))\).
Neither closes the gap.  The route-level conclusion is that temporal normal
forms are exhausted on the quotient route as they were on the pressure route
(HF01, HF04, HF10–HF13): the remaining mechanism must be a genuine
cancellation inside \(K=\int q\cdot\nabla\Pi_{u,A}\) on families with
\(\|q\|_3/\|w\|_3\) driven up (HF18-A audit R9), i.e. the FALSIFY action
already recorded in HF18-B, not another functional.
