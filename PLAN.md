...
source_small_bridge_purity: fixed-horizon-small-data-bridge-retains-order-epsilon-old-grade-purity-fails
...
The factor-two nonlinear branch has one additional exact triangular structure.
...
### 8.2 Approximate gaps and critical-form feedback, rather than Lipschitz action
...
Sobolev/interpolation consumer is now replayed there, while the
...
the domain dimension is `3` (the Jacobian is merely `ℝ⁹`-valued). The pinned target build now replays `HessianLaplacianL2`, the compact-support
adapter, and `GradientInterpolation`; the Gaussian, bump-function, and
operator-norm interpolation oracles pass. Astra review confirms that the
