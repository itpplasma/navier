...
\node[imported] (ESS) at (-13.20,-0.00) {\hyperref[node:ESS]{Escauriaza–Seregin–Šverák endpoint theorem}\\{\scriptsize IMPORTED}};
\node[paper] (LERAY-HOPF) at (-6.60,-7.80) {\hyperref[node:LERAY-HOPF]{Classical branch is a Leray–Hopf solution}\\{\scriptsize PAPER}};
...
\clearpage
\raggedright
Graph SHA-256: \texttt{23d681a352a98fb7e2028e09cb944112}\texttt{d22e60e24062b315f92119017ddf1a86}.
...
\textbf{Review:} Directly inspected at the published pages 52–53; transcribed in cp02-local-theory.md and checked in cp02-review-local-theory-r2.md. Not a statement about maximal development or about viscosity other than one.
...
\textbf{Review:} Directly inspected at the published pages 56–57; the passage from the per-horizon dichotomy to one maximal time is manuscript-owned (LOCAL).
...
\textbf{Mechanism:} stretching identity, Gagliardo–Nirenberg, Young.
...
\textbf{Review:} Complete on the page; machine-checked in navier-formal (scalar\_obstruction\_exists). Not a Navier–Stokes blowup solution.
...
\textbf{Mechanism:} L1 frequency bound of the low-pass Riesz multiplier and Cauchy–Schwarz in time.
...
\Needspace{10\baselineskip}\section*{ESS: Escauriaza–Seregin–Šverák endpoint theorem}\phantomsection\label{node:ESS}
\textbf{Class:} IMPORTED. \textbf{Formal:} not started.

ESS 2003 Theorem 1.3, verbatim, at unit viscosity on R3, with the Leray–Hopf weak solution conditions (1.3)–(1.7) and the mixed-norm convention, so that a Leray–Hopf weak solution on Q\_T that lies in L-infinity in time of L3 in space belongs to L5 of Q\_T and is smooth and unique on Q\_T.

\textbf{Mechanism:} published backward-uniqueness theorem.

\textbf{Review:} Directly inspected at the published pages 211–214 in cp02-continuation.md; the L3 in the theorem is the space norm and L\_\{3,infinity\} means L-infinity in time. GKP Theorem 4 is corroboration only.

\textbf{Evidence:} \texttt{research/evidence/cp01-literature-statements.md}. Manuscript label: \texttt{thm:ess}.

\textbf{Dependencies:} none. \hyperref[map]{Back to map}.

\Needspace{10\baselineskip}\section*{LERAY-HOPF: Classical branch is a Leray–Hopf solution}\phantomsection\label{node:LERAY-HOPF}
\textbf{Class:} PAPER. \textbf{Formal:} not started.

The unit-viscosity rescaling of the classical branch is a Leray–Hopf weak solution on every Q\_T with T at most the rescaled maximal time, in the exact sense of ESS conditions (1.3)–(1.7), with bounds uniform up to the maximal time.
...
\textbf{Review:} cp02-review-continuation-r2.md PASS; the solenoidal density lemma is proved in full via Biot–Savart, Hardy and mollification.
...
Every smooth divergence-free rapidly decreasing initial velocity on R3 has a global smooth unforced Navier–Stokes velocity and pressure of uniformly bounded kinetic energy, for every positive viscosity.
...
The Leray projection extends from L2 to a bounded operator on L3 of R3 that annihilates gradients; Calderón–Zygmund boundedness of the Riesz transforms.

\textbf{Mechanism:} Calderón–Zygmund theory.
...
\textbf{Review:} Open, and audited as NOT a weakening of known hypotheses. Because the defect is at most half the velocity gradient in L2 at every time, this is implied by the classical Ladyzhenskaya-Prodi-Serrin and Beirao da Veiga gradient criterion on the same critical line with a equal to 2 and s equal to 4, so it is not weaker than what the literature already knows; its interest is that it observes only one scalar derivative of the representative rather than the full velocity gradient. It lies in the existential-equivalence class, with the converse written out in rem:qe-defect-scope. What is unconditionally available from the energy identity is the square, not the fourth power. Whether a branch exists with divergent squared-enstrophy integral but finite defect integral is unsettled in both directions.
...
\textbf{Review:} Audited PASS WITH SCOPE in hf25-review-defect-criterion.md and imported with the mandatory scope remark rem:qe-defect-scope. Conditional consumer; DEFECT-L4 is unproved and is not weaker than classical criteria. No progress toward a proof is claimed by this node's presence.
